...
FluxRPC quickstart - 3 essential Solana RPC methods with TypeScript types.
...
| `getTransaction(sig)` | `{ found, success, fee, slot }` |

## Code Structure

```
src/index.ts
├── Types         → BalanceResult, BlockhashResult, TransactionResult
├── Configuration → API key validation, region
├── Connection    → Reusable Connection instance
├── Utilities     → isValidPublicKey, lamportsToSol
├── RPC Methods   → getBalance, getBlockhash, getTransaction
└── Demo          → Example usage
```
